	...
	public void updateMember() {
		...
		try {
			...
			while(rs.next()) {
				...
				//SHA256클래스의 getSha256()메소드를 사용해 원래의 비밀번호를 SHA-256방식으로 암호화
				String shaPass = sha.getSha256(orgPass.getBytes());
				
				//Sha-256방식으로 암호화된 값을 다시 BCrypt클래스의 hashpw()메소드를 사용해 bcrypt방식으로 암호하
				//Bcrypt.gensalt()메소드는 salt값을 난수로 사용해 생성
